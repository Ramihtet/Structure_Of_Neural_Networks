...
for (var r = 0; r < 1; r++) {
  ...
  for (var index = 0; index < tests; index++) {
    ...
    console.log('accurracy: {0}'.format((1 - errors / 1000) * 100));
    accuracies += acc; // console.log(network.connections)
  ...
  var d = JSON.stringify(model); // write JSON string to a file

  fs.writeFile('./models/model' + best_m_number.toString() + "_" + starting_model.toString() + '.json', d, function (err) {
    if (err) {
      throw err;
    }
  }); //LOAD
...
var xls = tool.json2xls(datas, {
  order: ["ID", "Rate", "Momentum", "Batchsize", "Rate_policy", "Dropout", "Activation", "Tests", "Best_Accuracy", "Av_Accuracy"]
});
