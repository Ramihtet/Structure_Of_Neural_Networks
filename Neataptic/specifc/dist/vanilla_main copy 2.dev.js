...
for (var index = 0; index < tests; index++) {
  ...
  for (var _i = 0; _i < 10; _i++) {
    eval("var core" + _i + " = new n.Node();");
    eval("nodes.push(core" + _i + ")");
  }

  for (var i = 0; i < input.length; i++) {
    for (var _k = 0; _k < loop1.length; _k++) {
      for (var j = 0; j < 10; j++) {
        eval("input[" + i + "].connect(loop" + j + "[" + _k + "]);");
      }
    }
  }
  ...
  for (var i = 0; i < 10; i++) {
    for (var j = 0; j < 10; j++) {
      // console.log("loop"+i+"["+j+"].connect(loop"+i+"["+next+"]);")
      eval("loop" + i + "[" + j + "].connect(core" + i + ");");
    }
  }

  for (var i = 0; i < output.length; i++) {
    for (var j = 0; j < 10; j++) {
      eval("core" + j + ".connect(output[" + i + "]);");
    }
  }
  ...
  for (var _i3 = 0; _i3 < 2000; _i3++) {
    if (g(Dataset[10000 + _i3]["output"]) != g(softmax(network.activate(Dataset[10000 + _i3]["input"])))) {
      errors = errors + 1;
    }
  }
